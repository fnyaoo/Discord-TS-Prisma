...
export default class Logger {
	...
	formatLog(text: string, options: FormatLogOptions) {
		...
		const finalMessage = this.makeSymmetric(
			options?.type === "error" ? index : (index.match(/\[(.*?)\]/)?.[1] as string),
			opts.maxIndexLength
		);

		const logMessage = `${finalMessage}${"[" + this.client?.logger.getCurrentTime() + "]"} ${message.trim()}`;
